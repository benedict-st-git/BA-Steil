...
"""
    Sampling 2.1: stratified- + memory sampling (RPLL(RPLineLengths) als Basis - 1 combination in samplings))
    -
    xxxx
"""

function get_hist_diagonal_stratified_and_memory_sampled(x::AbstractMatrix{T}, ε::T, M::Int) where {T<:AbstractFloat}
    N, dim = size(x)                  # Number of observations and variables
    L_local = zeros(Int, N)           # Histogram for line lengths
    L_local_old = zeros(Int, N)       # Histogram for line lengths
    ε2 = ε^2                          # Squared threshold (avoids sqrt)
